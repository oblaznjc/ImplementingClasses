...
class Point(object):
    ...
    def __init__(self, x, y):
        ...
        self.absolute_start_x = x
        self.absolute_start_y = y
    ...
    def move_by(self, dx, dy):
        if self.number_of_moves_made == 0:
            self.absolute_start = self.clone()
        start = self.clone()
        self.x = self.x + dx
        self.y = self.y + dy
        self.number_of_moves_made =  self.number_of_moves_made + 1
        self.distance_traveled = self.distance_traveled + self.get_distance_from(start)
    ...
    def get_distance_from(self, another_point):
        import math
        dx = abs(self.x - another_point.x)
        dy = abs(self.y - another_point.y)
        distance_from = math.sqrt(dx ** 2 + dy ** 2)
    ...
    def closer_to(self, point_1, point_2):
        if self.get_distance_from(point_1) <= self.get_distance_from(point_2):
            return point_1
        else:
            return point_2
